...
export class SyncManager {
    ...
    public async start(): Promise<void> {
        ...
        // Устанавливаем интервал для полной синхронизации
        if (this.options.fullSyncInterval) {
        ...
        // Выполняем начальную синхронизацию состояния
        await this.updateLocalFileState();
        
        // Запускаем полную синхронизацию после установления соединения
        // Задержка 5 секунд, чтобы дать время на установление соединения и получение списка доверенных устройств
        setTimeout(() => {
            console.log("Запуск начальной полной синхронизации после старта...");
    ...
    private async handleFileCreateOrModify(change: FileChange): Promise<void> {
        ...
        // Синхронизируем файл с доверенными устройствами
    ...
    private async handleFileDelete(change: FileChange): Promise<void> {
        ...
        // Синхронизируем удаление с доверенными устройствами
    ...
    private async handleFileRename(change: FileChange): Promise<void> {
        ...
        // Синхронизируем новый файл с доверенными устройствами
    ...
    private handleTrustedDevicesChange(devices: DeviceInfo[]): void {
        ...
        // Если есть доверенные устройства и давно не было синхронизации, запускаем полную синхронизацию
        if (hasDevices) {
            ...
            // Если прошло больше 5 минут с последней синхронизации
            if (timeSinceLastSync > 5 * 60 * 1000) {
                console.log("Давно не было синхронизации, запускаем полную синхронизацию");
            ...
            console.log("Нет доверенных устройств для синхронизации");
        }
    }
    ...
    public async performFullSync(): Promise<void> {
        try {
            ...
            if (!hasTrustedDevices) {
                console.log("Синхронизация пропущена: нет доверенных устройств");
                console.log("Текущие доверенные устройства:", JSON.stringify(trustedDevices));
                // Обновляем локальное состояние, чтобы список был готов, когда устройства появятся
                await this.updateLocalFileState();
                return;
            }
